...
func TestCalculateCPUPercentage(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "idle system",
			prev:     []float64{100, 0, 50, 9850, 0, 0, 0, 0}, // user, nice, system, idle, iowait, irq, softirq, steal
		...
		},
		{
			...
			expected: 15.0, // (3000 - 1500) / (20000 - 10000) = 1500/10000 = 15%
		},
		{
			name:     "100% usage",
		...
		},
		{
			...
			expected: 9.375, // busy increased by 750 (1500-750), total increased by 8000
		},
		{
			name:     "12.5% usage irq and softirq included",
			prev:     []float64{500, 0, 250, 7000, 0, 100, 150, 0},
			curr:     []float64{1000, 0, 500, 14000, 0, 200, 300, 0},
			expected: 12.5, // busy increased by 1000 (2000-1000), total increased by 8000
		},
		{
		...
		},
		{
			...
			expected: 0, // When total decreases, return 0
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.InDelta(t, tt.expected, result, 0.1, "CPU percentage mismatch")
		})
	}
}

func TestCalculateCPUPercentageRealWorld(t *testing.T) {
	// Simulate real CPU stats from a running system
	tests := []struct {
		name     string
		prev     []float64
		curr     []float64
		...
		},
		{
			...
			expected: 66.67, // Actual: (85000 - 75000) / (490000 - 475000) = 10000/15000 = 66.67%
			delta:    1.0,
		},
		{
			...
			expected: 80.0, // Actual: (172000 - 152000) / (327000 - 302000) = 20000/25000 = 80%
			delta:    1.0,
		},
	}
...
func TestReadThermalTemp(t *testing.T) {
	// Note: This test is more about ensuring the function handles errors correctly
	// Real filesystem testing would require mocking
